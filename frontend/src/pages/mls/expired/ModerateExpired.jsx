...
export const ModerateExpired = () => {
  ...
  const fetchListings = async () => {
    ...
    try {
      ...
      const response = await api.get(`/expired-listings/?${params}`);
  ...
  const handleApprove = async (listingId) => {
    try {
      await api.patch(`/expired-listings/${listingId}/status?status=approved`);
  ...
  const handleReject = async (listingId) => {
    try {
      await api.patch(`/expired-listings/${listingId}/status?status=rejected`);
  ...
  const handleConvertToLead = async (listingId) => {
    setConverting(true);
    try {
      const response = await api.post(`/expired-listings/${listingId}/convert-to-lead`);
      toast.success('Converted to Property Lead!');
  ...
  const handleBulkConvert = async () => {
    ...
    setConverting(true);
    try {
      const response = await api.post('/expired-listings/bulk-convert', selectedIds);
  ...
  return (
    <div className="space-y-6" data-testid="moderate-expired-page">
      {/* Header */}
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-serif font-bold flex items-center gap-2">
            <Clock className="w-6 h-6 text-orange-500" />
            Moderate Expired Listings
          </h1>
          <p className="text-muted-foreground">
            Review and convert expired listings to leads ({total} total)
          </p>
        </div>
      ...
      {listings.length === 0 ? (
        <Card>
          <CardContent className="p-8 text-center">
            ...
            <p className="text-muted-foreground mb-4">No expired listings found</p>
            <Button variant="outline" onClick={() => window.location.href = '/mls/expired/search'}>
          ...
          {listings.map((listing) => (
            <Card 
              ...
              className={`overflow-hidden hover:border-orange-500/50 transition-colors ${
                selectedIds.includes(listing.mls_id) ? 'ring-2 ring-orange-500' : ''
              }`}
            >
              {/* Image */}
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-orange-500">{formatPrice(selectedListing.list_price)}</p>
                </div>
                <div>
                ...
                {selectedListing.listing_agent && (
                  <div className="col-span-2">
                    <p className="text-sm text-muted-foreground mb-1">Previous Listing Agent</p>
                    <p className="text-sm">{selectedListing.listing_agent}</p>
                  </div>
                )}
...
export default ModerateExpired;
