...
export const ConvertedExpired = () => {
  ...
  const fetchListings = async () => {
    setLoading(true);
    try {
      const response = await api.get('/expired-listings/?status=converted&limit=100');
  ...
  return (
    <div className="space-y-6" data-testid="converted-expired-page">
      {/* Header */}
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Expired listings converted to property leads for follow-up ({total} total)
          </p>
        </div>
      ...
      {listings.length > 0 && (
        <Card className="border-green-500/30 bg-green-500/5">
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <p>
              <span className="font-medium">{total} expired listings</span> have been converted to property leads.
...
export default ConvertedExpired;
