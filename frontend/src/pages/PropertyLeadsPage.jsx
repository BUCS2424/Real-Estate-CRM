...
const PropertyLeadsPage = () => {
  ...
  return (
    <div className="space-y-6" data-testid="property-leads-page">
      ...
      {/* Tabs for Leads / Moderation */}
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="bg-muted/50">
          <TabsTrigger value="leads" className="flex items-center gap-2">
            <Building2 className="w-4 h-4" />All Leads
          </TabsTrigger>
          <TabsTrigger value="moderation" className="flex items-center gap-2">
            <ClipboardList className="w-4 h-4" />
            Moderation Queue
            {moderationStats?.pending > 0 && (
              <Badge className="ml-1 bg-orange-500 text-white">{moderationStats.pending}</Badge>
            )}
          </TabsTrigger>
        </TabsList>
      ...
      <EmailComposerModal
        ...
        recipientEmail={emailRecipient?.owner_email || emailRecipient?.email}
        recipientName={emailRecipient?.owner_name || emailRecipient?.address}
      />
    </div>
  );
};
