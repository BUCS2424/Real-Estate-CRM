      ...
      <Dialog open={showEmailModal} onOpenChange={(open) => { setShowEmailModal(open); if (!open) { setEmailSubject(''); setEmailBody(''); setEmailRecipient(null); } }}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Send an email to {emailRecipient?.owner_name || emailRecipient?.address}
            </DialogDescription>
          </DialogHeader>
          
          <div className="space-y-4">
            <div className="p-3 rounded-lg bg-muted/50">
              <p className="text-sm font-medium">{emailRecipient?.owner_name || emailRecipient?.address}</p>
              <p className="text-sm text-muted-foreground flex items-center gap-1">
                <Mail className="w-3 h-3" />
                {emailRecipient?.owner_email || emailRecipient?.email}
              </p>
            </div>
          ...
          <DialogFooter className="mt-4">
            ...
            <Button 
              ...
              disabled={!(emailRecipient?.owner_email || emailRecipient?.email)}
