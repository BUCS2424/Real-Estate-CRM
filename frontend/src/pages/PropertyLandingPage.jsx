...
import {
  Bed, Bath, Square, MapPin, Phone, Mail, Calendar, Car, Home,
  ChevronLeft, ChevronRight, X, Loader2, Check, Share2, Heart,
  Building2, DollarSign, Ruler, FileText, Play, ArrowLeft
} from 'lucide-react';
...
const tabClass = "data-[state=active]:bg-amber-500 data-[state=active]:text-[#0a1628] data-[state=active]:font-bold data-[state=active]:shadow-md data-[state=active]:shadow-amber-500/40 text-white/70 hover:text-amber-300 hover:bg-white/5 rounded-lg px-4 py-2 text-sm font-medium transition-all duration-200";
...
export const PropertyLandingPage = () => {
  ...
  useEffect(() => {
    ...
    const trackingId = params.get('tracking_id') ||
      (document.cookie.match(/(?:^|; )mls_tracking_id=([^;]+)/) || [])[1];
    if (!trackingId) return;
    analyticsAPI.startSession({ tracking_id: trackingId, landing_slug: slug, landing_url: window.location.href, user_agent: navigator.userAgent })
      .then(res => { setTrackingSessionId(res.data.session_id); setTrackingStart(Date.now()); })
      .catch(() => {});
  }, [pageData, slug]);

  // ── Analytics: end session on unload ────────────────────────────────────
  useEffect(() => {
    if (!trackingSessionId || !trackingStart) return;
    const API = (process.env.REACT_APP_BACKEND_URL || '').replace(/\/+$/, '');
    const handler = () => {
      const payload = JSON.stringify({ session_id: trackingSessionId, duration_seconds: Math.floor((Date.now() - trackingStart) / 1000) });
      if (navigator.sendBeacon) navigator.sendBeacon(`${API}/api/analytics/session/end`, new Blob([payload], { type: 'application/json' }));
      else fetch(`${API}/api/analytics/session/end`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: payload, keepalive: true });
    };
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      await landingPagesAPI.submitContactForm(slug, formData);
      setSubmitted(true);
  ...
  const handleShare = () => {
    if (navigator.share) navigator.share({ title: pageData?.listing?.address, url: window.location.href });
    else { navigator.clipboard.writeText(window.location.href); toast.success('Link copied!'); }
  };

  // ── Loading / Error ──────────────────────────────────────────────────────
  if (loading) return <div className="min-h-screen bg-[#0a1628] flex items-center justify-center"><Loader2 className="w-12 h-12 animate-spin text-amber-400" /></div>;
  ...
  // ── Data normalisation ───────────────────────────────────────────────────
  const { listing: raw, videos = [], virtual_tour_url, custom_headline, custom_description, additional_images = [] } = pageData;
  const p = raw || {};

  const normalizedImages = [
    ...(p.images || []).map((img, i) => typeof img === 'string' ? { url: img, id: `li-${i}` } : img),
    ...additional_images.map((img, i) => typeof img === 'string' ? { url: img, id: `ai-${i}` } : img),
  ].filter(img => img?.url);

  // ensure hero/primary photo appears first
  const heroUrl = p.hero_image_url || p.primary_photo;
  if (heroUrl && !normalizedImages.find(img => img.url === heroUrl)) {
    normalizedImages.unshift({ url: heroUrl, id: 'hero', is_primary: true });
  }

  const photos = normalizedImages.map(i => i.url);
  const price = typeof p.price === 'string' ? parseInt(p.price.replace(/[^0-9]/g, '')) : (p.price || 0);

  const seoTitle = custom_headline || (p.address ? `${p.address}${p.city ? `, ${p.city}` : ''}` : 'Property Listing');
  const seoDescription = custom_description || p.description || 'Explore this exclusive property.';

  const hasVideos = videos.length > 0;
  const hasTour = !!virtual_tour_url;
  ...
  return (
    <div className="min-h-screen bg-[#0a1628] text-white">
      <PublicSeoHead
        title={`${seoTitle}${price ? ` | ${fmt(price)}` : ''}`}
      ...
      {photos.length > 0 && (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-4">
          <div
            className="grid grid-cols-1 md:grid-cols-4 gap-2 rounded-xl overflow-hidden cursor-pointer"
          ...
          <div className="flex items-center justify-between mt-3 mb-2">
            <Link to="/showcase" className="text-amber-400 hover:text-amber-300 text-sm flex items-center gap-1.5">
              <ArrowLeft className="w-4 h-4" /> Back to Listings
            </Link>
            <div className="flex items-center gap-3">
              <button onClick={handleShare} className="flex items-center gap-1.5 text-white/60 hover:text-amber-400 text-sm">
                <Share2 className="w-4 h-4" /> Share
              </button>
              <button onClick={() => setSaved(!saved)} className={`flex items-center gap-1.5 text-sm ${saved ? 'text-red-400' : 'text-white/60 hover:text-red-400'}`}>
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="pt-2">
              <div className="flex items-start justify-between">
                <div>
                  {price > 0 && <p className="text-amber-400 text-3xl lg:text-4xl font-serif font-bold">{fmt(price)}</p>}
                  <h1 className="text-xl lg:text-2xl font-serif text-white mt-1">{custom_headline || p.address}</h1>
                  <p className="text-white/50 flex items-center gap-1.5 mt-1">
                    <MapPin className="w-4 h-4 text-amber-400/60" />
                    {p.city}{p.state ? `, ${p.state}` : ''}{p.zip_code ? ` ${p.zip_code}` : ''}
                  </p>
                </div>
                {p.status && (
                  <Badge className="bg-amber-400/20 text-amber-400 border-amber-400/30 text-sm px-3 py-1">
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab}>
              <div className="relative">
                ...
                <TabsList className="relative w-full justify-start overflow-x-auto bg-[#0b1d38] border-2 border-amber-500/40 rounded-xl p-1.5 h-auto gap-0.5 flex-wrap shadow-lg shadow-amber-500/10">
                  ...
                  <TabsTrigger value="details"    className={tabClass}>Property Details</TabsTrigger>
                  {hasFeatures && <TabsTrigger value="features" className={tabClass}>Features</TabsTrigger>}
                  {hasVideos   && <TabsTrigger value="videos"   className={tabClass}>Videos</TabsTrigger>}
                  {hasTour     && <TabsTrigger value="tour"     className={tabClass}>Virtual Tour</TabsTrigger>}
                </TabsList>
              </div>
              ...
              {hasVideos && (
                <TabsContent value="videos" className="mt-6 space-y-6">
                  <SectionTitle icon={Play}>Property Videos</SectionTitle>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {videos.map((video) => (
                      <div key={video.id || video.url} className="space-y-2">
                        {video.source === 'youtube' && getYouTubeId(video.url) && (
                          <div className="aspect-video rounded-xl overflow-hidden">
                            <iframe src={`https://www.youtube.com/embed/${getYouTubeId(video.url)}`} title={video.title || 'Video'} className="w-full h-full" allowFullScreen />
                          </div>
                        )}
                        {video.source === 'vimeo' && getVimeoId(video.url) && (
                          <div className="aspect-video rounded-xl overflow-hidden">
                            <iframe src={`https://player.vimeo.com/video/${getVimeoId(video.url)}`} title={video.title || 'Video'} className="w-full h-full" allowFullScreen />
                          </div>
                        )}
                        {(video.source === 'upload' || (!getYouTubeId(video.url) && !getVimeoId(video.url))) && (
                          <video src={video.url} controls className="w-full aspect-video rounded-xl" />
                        )}
              ...
              {hasTour && (
                <TabsContent value="tour" className="mt-6">
                  <SectionTitle icon={Home}>Virtual Tour</SectionTitle>
                  <div className="aspect-video rounded-xl overflow-hidden">
                    <iframe src={virtual_tour_url} title="Virtual Tour" className="w-full h-full" allowFullScreen />
                  </div>
                </TabsContent>
              )}
            </Tabs>
            ...
            {price > 0 && price < MORTGAGE_CALCULATOR_LIMIT && (
              <Card className="bg-white/5 border-white/10">
                <CardContent className="p-6">
                  ...
                  <MortgageCalculator
                    propertyPrice={price}
                    propertyAddress={`${p.address || ''}${p.city ? `, ${p.city}` : ''}${p.state ? `, ${p.state}` : ''}`}
                    embedded={true}
                  />
                </CardContent>
              </Card>
      ...
      {showInquiryForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-[#0d1f3c] border border-amber-400/20 rounded-2xl p-8 max-w-md w-full relative">
            ...
            <p className="text-white/60 text-sm mb-6">{p.address}{p.city ? `, ${p.city}` : ''}</p>
            {submitted ? (
              <div className="text-center py-8">
                <Check className="w-12 h-12 text-amber-400 mx-auto mb-3" />
