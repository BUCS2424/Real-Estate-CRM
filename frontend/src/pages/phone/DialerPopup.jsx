...
const DialerPopup = () => {
  ...
  const createCallLog = async (call) => {
    try {
      ...
      callLogIdRef.current = res.data.call?.id;
      console.log("Call log created:", res.data.call?.id, "→", remoteNumber, direction);
    } catch (e) {
  ...
  const uploadRecording = async (blob) => {
    try {
      ...
      if (fileUrl && callLogIdRef.current) {
        await apiClient.put(`/calls/${callLogIdRef.current}`, { recording_url: fileUrl });
