...
const apiClient = {
  get: (path, config) => {
    ...
    return api.get(`/telnyx${path}`, config);
  },
  post: (path, data, config) => {
    if (path.startsWith("/auth")) {
      return api.post(path, data, config);
    }
    return api.post(`/telnyx${path}`, data, config);
  },
  put: (path, data, config) => api.put(`/telnyx${path}`, data, config),
  delete: (path, config) => api.delete(`/telnyx${path}`, config),
};
...
const DialerPage = () => {
  ...
  const uploadRecording = async (blob) => {
    try {
      ...
      if (fileUrl) {
        console.log("Recording uploaded:", fileUrl);
        if (activeCall?.id) {
          await apiClient.put(`/calls/${activeCall.id}/recording`, {
  ...
  const loadRecentCalls = async () => {
    try {
      ...
      setRecentCalls((response.data.calls || []).slice(0, 5));
    } catch (error) {
  ...
  const handleCallEnded = async (call) => {
    ...
    try {
      await apiClient.post("/calls", {
        phone_number: call?.options?.destinationNumber || call?.options?.remoteCallerNumber || phoneNumber,
